  ...
  template:`<div class="jtk-schema-table jtk-schema-element">
    ...
    <div class="jtk-schema-columns">
      @for(column of obj['columns'];track column) {
        <db-column [obj]="column" [parent]="this"></db-column>
      }
    </div>
  </div>
  `
...
export class TableNodeComponent extends BaseEditableNodeComponent {
  ...
  addColumn():void {
    //
    // this is a helper method provided by JsPlumb's `BaseNodeComponent`. It will add a new port to the data
    // for the node this component represents.  Under the hood JsPlumb does two things in response to this:
    //
    ...
    //
    // 2. it registers the new port data on the node in JsPlumb's data model.
    //
